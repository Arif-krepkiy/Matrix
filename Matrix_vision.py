...
class Matrix:
    def __init__(self,app, font_size = 12):
        ...
        self.katakana = np.array([chr(int('0x30a0',16) + i) for i in range(96)] + ['' for i in range(30)])
        self.font = pg.font.Font('MS Mincho.ttf', font_size, bold = True)

        self.matrix = np.random.choice(self.katakana, self.SIZE)
        self.char_intervals = np.random.randint(25,70, size = self.SIZE)
        self.cols_speed = np.random.randint(0,500, size=self.SIZE)
        self.get_prerendered_chars = self.get_prerendered_chars()

        self.image = self.get_image('ARIF.jpg')
            ...
            for x, char in enumerate(row):
                if char:
                    ...
                    if red and green and blue:
                        ...
                        color = 220 if 1660 < color < 220 else color
                        char = self.get_prerendered_chars[(char,(0,color,0))]
